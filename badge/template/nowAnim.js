...
`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${data.width}" height="20">
  ...
  <g mask="url(#a)">
    ...
    <path id="m1" fill="${data.from}" d="M${data.subjectWidth} 0 h${data.status1Width} v20 h-${data.status1Width} z"/>
    <path fill="${data.color2}" d="M${data.subjectWidth + data.status1Width} 0 h${data.status2Width} v20 h-${data.status2Width} z"/>
    <path fill="${data.color3}" d="M${data.subjectWidth + data.status1Width + data.status2Width} 0 h${data.status3Width} v20 h-${data.status3Width} z"/>
  </g>
  ...
  <g fill="#fff" text-anchor="middle" font-family="'Lucida Console'" font-size="11">
    ...
    <text x="${Math.floor(data.status3Width / 2) + data.subjectWidth + data.status1Width + data.status2Width}" y="15" fill="#010101" fill-opacity=".3">${data.status3}</text>
    <text x="${Math.floor(data.status3Width / 2) + data.subjectWidth + data.status1Width + data.status2Width}" y="14">${data.status3}</text>
  </g>
</svg>`);
};
